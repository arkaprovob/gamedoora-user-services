...
ssh -i "/tmp/deploy.key" -o StrictHostKeyChecking=no "$SSH_USERNAME@$SSH_HOST" "
...
scp -i "/tmp/deploy.key" -o StrictHostKeyChecking=no $JAR_FILE "$SSH_USERNAME@$SSH_HOST:$REMOTE_DIRECTORY/$REMOTE_JAR_FILE"
